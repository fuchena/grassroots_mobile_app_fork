...
class _NewObservationPageState extends State<NewObservationPage> {
  ...
  void initState() {
    ...
    selectedTraitKey =
        widget.selectedTraitKey; // Initialize with passed trait key
  ...
  Future<void> _initRetrievePhoto() async {
    if (plotNumber == null) {
      // try to extract plotNumber from widget if not set yet
      try {
        plotNumber = widget.plotDetails['rows']?[0]['study_index'] as int?;
      } catch (_) {}
    }

    var result = await ApiRequests.retrieveLastestPhoto(
        studyID ?? 'defaultFolder', plotNumber ?? 0);
    ...
    if (result['status'] == 'success') {
      if (mounted) {
        setState(() {
          ...
          _photoDate =
          dateStr != null ? DateFormat('yyyy_MM_dd').parse(dateStr) : null;
        });
      }
  ...
  void _handleUpload() async {
    if (_image != null && studyID != null && plotNumber != null) {
      ...
      try {
        ...
        bool uploadSuccess = await ApiRequests.uploadImageDate(
            savedImage, studyID!, plotNumber!);
        ...
        setState(() {
          ...
          if (uploadSuccess) {
            ...
            _photoDate =
                DateTime.now(); // Set the current date as the photo date
          }
        });
        ...
        if (uploadSuccess) {
          final photoSubmission = PhotoSubmission(
            ...
            syncStatus: backendRequests
                .SYNCED, // Mark as synced since the upload succeeded
          );
  ...
  void _extractPhenotypeDetails() {
    final data = widget.studyDetails['results']?[0]['results']?[0]['data'];
    if (data == null) return;

    if (data.containsKey('phenotypes')) {
      var phenotypes = data['phenotypes'] as Map<String, dynamic>;
    ...
    // Extract the studyID
    try {
      studyID = widget.studyDetails['results'][0]['results'][0]['data']['_id']
      ['\$oid'];
  ...
  void moveToPlot(bool forwardsFlag) async {
    ...
    var plots = widget.studyDetails['results']?[0]['results']?[0]['data']
              ...
              onPressed: () async {
                ...
                if (newMax != null && newMin != null && newMin < newMax) {
                  bool updated = await ApiRequests.updateLimits(
                      studyID ?? 'defaultFolder',
                      newMin,
                      newMax,
                      selectedTraitKey ?? 'default_trait');
  ...
  Future<void> _submitObservation() async {
    if (_formKey.currentState!.validate()) {
      ...
      String dateString = DateFormat('yyyy-MM-dd')
          .format(selectedDate ?? DateTime.now()); // Date
      String? note = _notesEditingController.text.isEmpty
          ? null
          : _notesEditingController.text; // Notes
              ...
              children: <Widget>[
                ...
                Row(
                  crossAxisAlignment: CrossAxisAlignment
                      .end, // Align the TextFormField and Text vertically
                  children: [
                    Expanded(
                      child: TextFormField(
                        controller: _valueController,
                        readOnly: units[selectedTraitKey] ==
                            'yyyymmdd', // Make field read-only when unit is 'yyyymmdd'
                        keyboardType: units[selectedTraitKey] == 'day'
                            ? TextInputType.number
                            : TextInputType.datetime,
                        decoration: InputDecoration(
                          labelText: units[selectedTraitKey] == 'yyyymmdd'
                              ? 'Select date'
                              : 'Enter value',
                        ...
                        validator: (value) {
                          ...
                          if (minLimits[selectedTraitKey] != null &&
                              maxLimits[selectedTraitKey] != null) {
                    ...
                    if (units[selectedTraitKey] != null &&
                        units[selectedTraitKey]!.isNotEmpty)
                      Padding(
                ...
                ListTile(
                  ...
                  subtitle: Text(
                    selectedDate != null
                        ...
                        : 'Press to choose different date',
                  ),
                  onTap: () async {
                    final DateTime? picked = await showDatePicker(
                      context: context,
...
// Arrow buttons container (Previous and Next)
                    Expanded(
                      flex:
                      1, // The arrow buttons will occupy the remaining half of the row
                      child: Column(
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment
                                .spaceBetween, // Spread out the arrow buttons
                            children: [
                              ...
                              Column(
                                children: [
                                  IconButton(
                                    onPressed: () {
                                      moveToPlot(
                                          false); // Function to go to the previous plot
                                      print('Previous Plot');
                                    },
                                    icon: Icon(
                                      Icons.arrow_back,
                              ...
                              Column(
                                children: [
                                  IconButton(
                                    onPressed: () {
                                      moveToPlot(
                                          true); // Function to go to the next plot
                                    },
                                    icon: Icon(
                                      Icons.arrow_forward,
                                      color: Theme.of(context).primaryColor,
                      ...
                      if (_image != null)
                        GestureDetector(
                          onTap: () {
                            ...
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (context) => FullSizeImageScreenFile(
                                    imageFile: _image, plotNumber: plotNumber),
                              ),
                            );
                          },
                          child: Hero(
